...
var chartDom = document.getElementById("map4A");
...
const legendData = Object.keys(dataByPart).map(part => `${part} (${dataByPart[part].length})`);
  ...
  .then((geojson) => {
    ...
    option = {
      ...
      series: [
        ...
        ...Object.keys(dataByPart).map((part, index) => ({
          name: part,
          type: 'scatter',
          coordinateSystem: 'geo',
           symbol: 'pin',
          ...
          itemStyle: {
            color: ['#5c0569'][index],
            //color: ['#905cec', '#33cc33', '#5DBD32', '#04B9FF'][index], // Unique color for each part
          ...
          tooltip: {
            ...
            formatter: function (params) {
              if (params.seriesType === 'scatter') {
                return `${params.value[4]}<br/>服務量: ${params.value[2]}`;
              }
            }
          },
          data: dataByPart[part],
